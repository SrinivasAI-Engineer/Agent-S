...
import ideyalabsLogo from "../public/ideyalabs-logo.png";
...
function AuthView({ onLogin }: { onLogin: (token: string) => void }) {
  ...
  return (
    <div className="appShell" style={{ background: "var(--grey-sidebar)" }}>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div className="card" style={{ maxWidth: 400, width: "100%" }}>
          <div className="detailTitle" style={{ marginBottom: 16 }}>{signup ? "Sign up" : "Sign in"}</div>
          {err ? <div className="muted" style={{ color: "#b91c1c", marginBottom: 8, fontSize: 14 }}>{err}</div> : null}
          <div style={{ display: "grid", gap: 10 }}>
            ...
            />
            <button className="primary" disabled={loading || !email || !password} onClick={submit}>
...
function MainApp({ token }: { token: string }) {
  ...
  const refreshInbox = useCallback((): Promise<void> => {
    ...
    setErr("");
    return apiGet<ExecutionSummary[]>("/v1/inbox", token)
      .then((data) => {
        setInbox(data);
      ...
      <header className="header">
        <div className="headerLeft">
          ...
          <nav className="headerNav">
            <span className="headerNavTab active">Agent Inbox</span>
          </nav>
        </div>
        <div className="headerRight">
          <div className="headerUser">
            <div className="headerUserAvatar" title={user.email}>
              {(user.email || "?").charAt(0).toUpperCase()}
            </div>
            <div>
              <div>{user.email}</div>
              <div className="headerUserEmail">Signed in</div>
            </div>
          </div>
          <button type="button" className="headerLogout" onClick={() => { localStorage.removeItem(TOKEN_KEY); window.location.reload(); }}>Logout</button>
        </div>
      </header>

      <div className="layout">
        <div className="sidebar">
          <div className="sidebarPill">AGENT SOCIAL</div>
          <div className="sidebarSectionTitle">Features</div>

          <div className="card" style={{ marginBottom: 14 }}>
            <div className="cardTitle">New URL</div>
            <div style={{ display: "grid", gap: 10 }}>
              <input value={url} onChange={(e) => setUrl(e.target.value)} type="text" placeholder="https://example.com/article" />
              <button className="primary" disabled={loading || !url} onClick={createExecution}>Create execution</button>
            </div>
          </div>

          <div className="card" style={{ marginBottom: 14 }}>
            <div className="cardTitle">Connections</div>
            <div className="cardSubtitle">Add multiple Twitter/LinkedIn accounts; set labels and default.</div>
            {connections.length === 0 ? (
              <div className="muted" style={{ marginBottom: 8 }}>No accounts connected yet.</div>
            ) : (
              <ul style={{ listStyle: "none", padding: 0, margin: "0 0 10px 0" }}>
                {connections.map((c) => (
                  <li key={c.id} style={{ marginBottom: 8, padding: "6px 0", borderBottom: "1px solid var(--grey-border)" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      <span style={{ fontWeight: 600, color: "var(--corporate-blue)" }}>{c.provider === "twitter" ? "Twitter" : "LinkedIn"}</span>
                      {editingLabelId === c.id ? (
                        <>
                          <input
                            ...
                            style={{ width: 100 }}
                          />
                          <button type="button" className="primary" onClick={() => saveLabel(c.id)}>Save</button>
          ...
          <div className="sidebarSectionTitle">Agent Inbox</div>
          <div className="topBar" style={{ marginBottom: 10 }}>
            <span className="cardTitle" style={{ marginBottom: 0 }}>Executions</span>
            <button className="secondary" onClick={() => refreshInbox().catch((e: unknown) => setErr(String(e)))}>Refresh</button>
          </div>
          {inbox.map((x) => (
            <div
              key={x.execution_id}
            ...
            >
              <div className="listItemTitle">{x.status}</div>
              <div className="listItemMeta">{x.updated_at}</div>
        ...
        <div className="main">
          <div className="card">
            <div className="topBar">
              <div>
                <div className="detailTitle">Execution Details</div>
                <div className="detailSubtitle">{selectedId || "None selected"} · Review, annotate, and prepare for posting</div>
              </div>
              <div className="muted">{selected?.status ?? ""}</div>
            </div>

            {err ? (
              <div className="card cardError" style={{ marginBottom: 14 }}>
                <div className="cardTitle" style={{ marginBottom: 6 }}>Error</div>
            ...
            {selected ? (
              <>
                ...
                {selected.status === "running" ? (
                  <div className="card cardInfo" style={{ marginBottom: 14 }}>
                    <div className="cardTitle" style={{ marginBottom: 6 }}>Execution in Progress</div>
                ...
                {selected.status === "terminated" && selectedState.terminate_reason ? (
                  <div className="card cardError" style={{ marginBottom: 14 }}>
                    <div className="cardTitle" style={{ marginBottom: 6 }}>Execution Terminated</div>
                ...
                {(selected.status === "awaiting_human" || selected.status === "awaiting_auth" || selected.status === "completed") ? (
                  <div className="row" style={{ marginBottom: 14 }}>
                    <div className="card">
                      <div className="cardTitle" style={{ marginBottom: 8 }}>Generated Twitter post</div>
                      <textarea value={editedTwitter} onChange={(e) => setEditedTwitter(e.target.value)} placeholder="No draft yet" />
                    </div>
                    <div className="card">
                      <div className="cardTitle" style={{ marginBottom: 8 }}>Generated LinkedIn post</div>
                ...
                {(selected.status === "awaiting_human" || selected.status === "awaiting_auth") ? (
                  <div className="row" style={{ marginBottom: 14 }}>
                    <div className="card">
                      <div className="cardTitle" style={{ marginBottom: 10 }}>Selected image</div>
                      {img?.image_url ? (
                        <>
                    ...
                    <div className="card">
                      <div className="cardTitle" style={{ marginBottom: 10 }}>Actions</div>
                      ...
                      <div style={{ display: "flex", gap: 10 }}>
                        <button className="primary" disabled={loading} onClick={submitActions}>Submit actions</button>
